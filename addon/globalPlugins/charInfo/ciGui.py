...
class CharInfoSettingsPanel(settingsDialogs.SettingsPanel):
	...
	def makeSettings(self, sizer):
		...
		if not set(configChoices) == set(o for (o, __s) in self.actionNPressesLabels):
	...
	def makeActionNPressCombobox(self, nPresses):
		...
		actionNPressesChoices = [name for _setting, name in self.actionNPressesLabels]
		actionNPressesCombobox = self.actionGroup.addLabeledControl(
			actionNPressesText,
			wx.Choice,
			choices=actionNPressesChoices,
		)
		for index, (setting, _name) in enumerate(self.actionNPressesLabels):
